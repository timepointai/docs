...
Used for direct service-to-service calls that bypass the Gateway (e.g. Billing to Flash, Pro to Clockchain):

```bash
curl -H "X-Service-Key: YOUR_KEY" \
  https://billing.timepointai.com/api/v1/credits/balance
```

### X-Admin-Key (Admin Operations)

Used for privileged admin operations on backend services:

```bash
curl -H "X-Admin-Key: YOUR_ADMIN_KEY" \
  https://clockchain.timepointai.com/admin/reindex
```
...
3. Gateway proxies the request to the backend service (e.g. Flash) with an `X-User-ID` header
4. The backend service trusts `X-User-ID` — it does not validate the JWT itself

Flash has `AUTH_ENABLED=false`. It does not perform any authentication. It trusts the `X-User-ID` header injected by the Gateway.

## Service Keys

| Key | Used By |
|-----|---------|
| `FLASH_SERVICE_KEY` | Service-to-service calls to Flash |
| `SERVICE_API_KEY` | Clockchain authenticated endpoints |
| `PRO_API_KEY` | Pro Cloud API |
| `ADMIN_KEY` | Clockchain admin operations (via `X-Admin-Key`) |
| `WRITER_TOKENS` | Clockchain write operations (per-writer identity tokens) |

## Auth by Service

| Service | End-User Auth | Service-to-Service | Notes |
|---------|--------------|-------------------|-------|
| API Gateway (`api.timepointai.com`) | Bearer JWT, API keys | N/A | Auth authority — validates all consumer auth |
| Flash (`flash.timepointai.com`) | None (AUTH_ENABLED=false) | `X-Service-Key` | Trusts `X-User-ID` from Gateway |
| Clockchain (`clockchain.timepointai.com`) | None (public reads) | `WRITER_TOKENS` (writes), `X-Admin-Key` (admin) | Public read, authenticated write |
| Clockchain MCP (`clockchain.timepointai.com/mcp/`) | None (public reads) | N/A | Streamable HTTP, v1.26.0 |
| Pro Cloud (`pro.timepointai.com`) | Gateway JWT (introspection) | `X-Service-Key` or `INTERNAL_API_KEY` | Validates JWTs via Gateway |
| Billing (`billing.timepointai.com`) | N/A (internal) | `X-Service-Key` | Internal service only |
...
| `/internal/credits/*` | Internal credit operations (blocked from external access) |
