...
### GET /health
Instant liveness probe — no I/O. Returns service status and configured provider availability.
...
### GET /health/deep
Deep health check — verifies database connectivity and provider configuration.

```bash
GET /health/deep
```

No auth required. Performs a live database connection check (5s timeout) and returns `HealthResponse` with actual connectivity status.
