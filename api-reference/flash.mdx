...
The `X-User-ID` header is forwarded to Clockchain for all entity operations (search, resolution, grounding, and re-grounding). This enables visibility filtering, allowing users to see and reuse their own private entities during generation and search.
