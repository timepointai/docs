...
The `X-User-ID` header is forwarded to Clockchain during entity searches, generation, and background grounding. This enables visibility filtering so users can see their own private entities and shared entities in search results and generation pipelines.
